...
COMMENT ON COLUMN identidad.personas.ci_validada IS 'TRUE cuando la cédula pasó el algoritmo del dígito verificador ecuatoriano (módulo 10 del Registro Civil, implementado en CedulaEcuatoriana). FALSE cuando: (a) el documento no es una cédula ecuatoriana (pasaporte, RUC, doc. extranjero), (b) la cédula no pasó el módulo 10, o (c) la persona fue creada por una ruta que aún no calcula el flag. Lo puebla el INSERT de platform-service (auto-register wizard), auth-service (PersonaMapper: registro OAuth PWA, POST /personas, y RECALCULA en UPDATE de PUT /personas) y core-service (PersonaPersistenceAdapter: registro de cliente desde el panel admin). Pendientes: backfill de personas previas a 2026-07-14 y exposición REST del flag — ver docs/gym-administrator/pendientes/validacion-cedula-persona.md.';
