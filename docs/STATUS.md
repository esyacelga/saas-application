...
| Documento | Estado |
|-----------|--------|
| roadmap-sri-2026.md | 📋 Roadmap 2026-07-11 — **🔴 División en 6 fases con dependencias.** Punto de entrada para planificar. Fase 0 Fundamentos (G5 secuencial + G6 catálogos). Fase 1 Pipeline SRI (G2 tiempo real + G1 XML v2.2.0). Fase 2 Ciclo de vida (G4 NC + G3 anulación). Fase 3 Cumplimiento (G10 bancarización + G9 ATS). Fase 4 Complementarios opcionales (G7 ND + G8 retención + G13 guías). Fase 5 Rediseño frontend. Fase 6 Diferibles (G11 QR + G12 sync finanzas). El desarrollo arranca por Fase 0. |
