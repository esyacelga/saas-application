...
| [error-contract.md](architecture/error-contract.md) | Contrato de errores estandarizado (RFC 7807 + `codigo`) — ✅ implementado en los 6 microservicios + frontend |
...
| [solicitudes-membresia.md](requirements/solicitudes-membresia.md) | 📋 **Pendiente — spec definida (2026-07-19)** — HU cross-cutting: cliente PWA sin membresía activa ve catálogo y envía solicitud → staff ve widget contador en dashboard + fila destacada en Ventas Pendientes con modal "Completar venta" precargado. Reutiliza `core.membresias` con nueva columna `origen ('cliente'\|'staff')` — sin tabla nueva. 2 endpoints backend nuevos (`POST /clientes/me/membresias/solicitar`, `GET /pendientes/contador`) + extensión de `POST /membresias/{id}/confirmar-pago`. 5 decisiones de PO cerradas (precio actual, no acceso con pendiente, solo staff cancela, solo staff elige método). Se implementa junto con HU previa `estado-pago-membresias` (comparten migración). Sub-docs: [`docs/core-service/spec/solicitudes-membresia.md`](../core-service/spec/solicitudes-membresia.md), [`docs/gym-member-pwa/spec-solicitud-membresia.md`](../gym-member-pwa/spec-solicitud-membresia.md), [`docs/auth-service-frond-end/spec-solicitudes-membresia.md`](../auth-service-frond-end/spec-solicitudes-membresia.md). |
