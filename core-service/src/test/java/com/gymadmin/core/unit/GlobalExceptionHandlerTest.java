...
import com.gymadmin.core.infrastructure.config.GlobalExceptionHandler;
...
class GlobalExceptionHandlerTest {
    ...
    void notFound() {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/v1/clientes/9").build());
        StepVerifier.create(handler.handle(exchange, new NotFoundException("Cliente no existe")))
                .verifyComplete();
        ...
        assertThat(json.get("detail").asText()).isEqualTo("Cliente no existe");
        assertThat(json.get("mensaje").asText()).isEqualTo("Cliente no existe"); // alias
        assertThat(json.get("instance").asText()).isEqualTo("/api/v1/clientes/9");
    ...
    @Test
    @DisplayName("LimiteAlcanzadoException → 403 codigo=limite_plan_alcanzado + metadata snake_case")
    void limitePlan() {
        JsonNode json = handleAndParse(
                new LimiteAlcanzadoException("clientes", 50, 50, "FREE"));
        assertThat(json.get("status").asInt()).isEqualTo(403);
        assertThat(json.get("codigo").asText()).isEqualTo("limite_plan_alcanzado");
        assertThat(json.get("recurso").asText()).isEqualTo("clientes");
        assertThat(json.get("actual").asLong()).isEqualTo(50);
        assertThat(json.get("maximo").asLong()).isEqualTo(50);
    ...
    void uniqueViolation() {
        DataIntegrityViolationException ex = new DataIntegrityViolationException(
                "insert failed",
                new RuntimeException("ERROR: duplicate key value violates unique constraint \"uq_clientes_correo\""));
        JsonNode json = handleAndParse(ex);
        assertThat(json.get("status").asInt()).isEqualTo(409);
        assertThat(json.get("codigo").asText()).isEqualTo("datos_duplicados");
        assertThat(json.get("detail").asText()).contains("uq_clientes_correo");
    }
    ...
    void fkViolation() {
        DataIntegrityViolationException ex = new DataIntegrityViolationException(
                "insert failed",
                new RuntimeException("ERROR: insert violates foreign key constraint \"fk_membresia_cliente\""));
        JsonNode json = handleAndParse(ex);
