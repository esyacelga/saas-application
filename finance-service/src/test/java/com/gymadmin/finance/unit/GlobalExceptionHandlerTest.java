...
import com.gymadmin.finance.infrastructure.config.GlobalExceptionHandler;
...
class GlobalExceptionHandlerTest {
    ...
    private JsonNode handleAndParse(Throwable ex) {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/v1/finanzas/ingresos").build());
    ...
    void notFound() {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/v1/finanzas/ingresos/9").build());
        StepVerifier.create(handler.handle(exchange, new NotFoundException("Ingreso no existe")))
                .verifyComplete();
        ...
        assertThat(json.get("detail").asText()).isEqualTo("Ingreso no existe");
        assertThat(json.get("mensaje").asText()).isEqualTo("Ingreso no existe"); // alias
        assertThat(json.get("instance").asText()).isEqualTo("/api/v1/finanzas/ingresos/9");
        assertThat(json.hasNonNull("timestamp")).isTrue();
    }

    @Test
    @DisplayName("ConflictException (codigo null) → 409 codigo=conflicto")
    void conflict() {
    ...
    @Test
    @DisplayName("ConflictException con codigo propio → 409 preserva ese codigo")
    void conflictPreservaCodigoPropio() {
        JsonNode json = handleAndParse(new ConflictException("categoria_en_uso", "La categoría está en uso"));
        assertThat(json.get("status").asInt()).isEqualTo(409);
        assertThat(json.get("codigo").asText()).isEqualTo("categoria_en_uso");
        assertThat(json.get("detail").asText()).isEqualTo("La categoría está en uso");
    }
    ...
    void uniqueViolation() {
        DataIntegrityViolationException ex = new DataIntegrityViolationException(
                "insert failed",
                new RuntimeException("ERROR: duplicate key value violates unique constraint \"uq_categorias_ingreso_nombre\""));
        JsonNode json = handleAndParse(ex);
        assertThat(json.get("status").asInt()).isEqualTo(409);
        assertThat(json.get("codigo").asText()).isEqualTo("datos_duplicados");
        assertThat(json.get("detail").asText()).contains("uq_categorias_ingreso_nombre");
    }
    ...
    void fkViolation() {
        DataIntegrityViolationException ex = new DataIntegrityViolationException(
                "insert failed",
                new RuntimeException("ERROR: insert violates foreign key constraint \"fk_ingreso_categoria\""));
        JsonNode json = handleAndParse(ex);
