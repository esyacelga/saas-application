...
Key route groups:
- `CompaniaController` → `/api/v1/companias` — CRUD + wizard registration (`POST /wizard`) + auto-registro + logo upload (`POST /{id}/logo`) + suspend (`PUT /{id}/suspender`) + recordatorio de vencimiento WhatsApp (`POST /{id}/recordatorio-vencimiento`). El recordatorio (GYM-002, `requirePlataforma`) es un **envío directo e inmediato** (no encola) del aviso de vencimiento al dueño; reglas mínimas: opt-in `acepta_whatsapp` (→422 `no_consentimiento`) + teléfono normalizable a E.164 (→422 `telefono_invalido`) + suscripción activa (→422 `sin_suscripcion`; 404 si la compañía no existe). Elige la plantilla por días reales restantes (`ChronoUnit.DAYS.between(hoy, fechaFin)`): día 0 → `venc_suscripcion_hoy`, cualquier otro incluidos negativos/vencidos → `recordatorio_vencimiento_suscripcion`. Reutiliza la maquinaria de `WhatsAppQueueService` (mismo sender/params); permite cualquier plan. Respuesta 200 `{ enviado, telefono, template }`.

  **Idempotencia (cada mensaje de WhatsApp tiene costo).** Antes de enviar consulta `NotificacionRepository.fechaEnvioPrevio(idCompaniaPlan, tipo, 'whatsapp', bucket)` sobre la MISMA tabla `tenant.notificaciones_suscripcion` que usa el job automático, con el mismo predicado que `existsIdempotente` (`dias_antes >= bucket AND estado <> 'fallido'`). Si ya hay un aviso → **409 `notificacion_ya_enviada`** con la extensión `fecha_envio_previo`; el frontend muestra "ya se envió el {fecha}" y ofrece reintentar con `?forzar=true`. Tras un envío exitoso **registra una fila** (`estado='enviado'`, `fecha_envio`, `tipo` = `VENCIMIENTO_TRIAL`/`VENCIMIENTO_PREMIUM` según el código del plan, `dias_antes` = `max(diasRestantes, 0)`), lo que además evita que el job automático repita el mismo aviso después. El registro va dentro de `Mono.defer` para que un fallo del sender **no** deje constancia de un mensaje que nunca se cobró (así el operador puede reintentar sin forzar). No se creó tabla nueva: el modelo por-evento existente es lo que permite auditar el costo. El listado (`GET /api/v1/companias`) enriquece cada compañía con su suscripción vigente: `planActivo: { nombre, estado (MAYÚSCULA, igual que `SuscripcionResponse` — vía `Estado.name()`), fechaFin, diasRestantes }` o `null` si no hay suscripción activa. El use case `listarCompanias` devuelve `Flux<CompaniaConPlan>` (VO de dominio que agrupa `Compania` + plan activo); el enriquecimiento vive en `CompaniaService` (por compañía: `findActivoByIdCompania` + `PlanRepository.findById` para el nombre). Los endpoints de una sola compañía (`GET/PUT /{id}`, logo) devuelven `planActivo: null` (sin contexto de plan).
